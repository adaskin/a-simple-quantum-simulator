...
def prob_of_a_qubit(psi, qubit):
    ...
    for ti in range(nthreads):
        ...
        p = mp.Process(
            target=worker_prob_of_a_qubit, args=(psi, start, end, qshift, fshared)
        )
        processes.append(p)
        p.start()
...
def prob_of_qubits(psi, qubits):
    ...
    for ti in range(nthreads):
        ...
        p = mp.Process(
            target=worker_prob_of_qubits, args=(psi, n, qubits, start, end, fshared)
        )
        processes.append(p)
        p.start()
...
def worker_apply_gate_to_state(
    psi_shared, start, end, Gate, target, cqsorted, nqubit, qshift, lock=None
):
...
def apply_gate_to_state(psi_shared, Gate, target, control_qubits=[]):
    ...
    for ti in range(nthreads):
        ...
        p = mp.Process(
            target=worker_apply_gate_to_state,
            args=(psi_shared, start, end, Gate, target, cqsorted, nqubit, qshift, lock),
        )
        processes.append(p)
        p.start()
